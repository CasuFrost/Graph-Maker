...
public:
    ...
    void createRandomGraph(int nodesNumber, int p)
    {
        ...
        for (int i = 0; i < nodesNumber; i++)
        {
            for (int j = 0; j < nodesNumber; j++)
            {
                ...
                if (randInt < p && i != j && !isNodeInVector(nodes[i], nodes[j].adjacent))
                { // Prob(i-j)= (p/nodesNumber)
    ...
    void setupGraph()
    { // This function fill in the correct way  the adjacent list of all the nodes
        for (int i = 0; i < nodes.size(); i++)
        {
    ...
    bool isIntInVector(int a, vector<int> b)
    { // Return true if a is in the adjiacent list of b
    ...
    bool isNodeInVector(Node a, vector<Node> b)
    { // Return true if a is in the adjiacent list of b
        for (int i = 0; i < b.size(); i++)
        {
            if (b[i].id == a.id)
